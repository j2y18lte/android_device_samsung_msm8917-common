...
#endif
{
    ...
    /*
     * J415 emits Samsung-private notifications with no AOSP consumer.
     * 11010 is the legacy AM command hook, 22009 accompanies the normal
     * signal-strength indication, and 42010 is its DSDS/private companion.
     * Required framework indications arrive separately.
     */
    if (unsolResponse == RIL_UNSOL_AM ||
            unsolResponse == 22009 ||
            unsolResponse == 42010) {
        RLOGV("Ignoring Samsung private unsolicited response %d", unsolResponse);
        return;
    }
