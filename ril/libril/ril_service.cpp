...
void convertRilCellInfoListToHal(
        void *response, size_t responseLen, hidl_vec<CellInfo>& records);

static_assert(sizeof(RIL_CellInfo_v12) == 1104,
        "J415 RIL_CellInfo_v12 ABI must be 1104 bytes");
static_assert(offsetof(RIL_CellInfo_v12, CellInfo) == 28,
        "J415 RIL_CellInfo_v12 union offset must be 28 bytes");
...
/*
 * Some Samsung Qualcomm RIL blobs return the normal AOSP RIL_AppStatus
 * layout, while older Samsung blobs append five retry counters to every
 * application entry. Keep the device Samsung ril.h, but accept both wire
 * layouts at the libril boundary.
 */
struct RIL_AppStatus_AOSP_Compat {
...
struct RIL_CardStatus_v6_AOSP_Compat {
    ...
    RIL_AppStatus_AOSP_Compat applications[RIL_CARD_MAX_APPS];
};

struct RIL_CardStatus_Header_Compat {
    RIL_CardState card_state;
    RIL_PinState universal_pin_state;
    int gsm_umts_subscription_app_index;
    int cdma_subscription_app_index;
...
int radio::getIccCardStatusResponse(int slotId,
    ...
    if (radioService[slotId]->mRadioResponse != NULL) {
        ...
        const size_t aospSize = sizeof(RIL_CardStatus_v6_AOSP_Compat);
        const size_t samsungSize = sizeof(RIL_CardStatus_v6);

        RLOGV("ICC ABI slot=%d response=%p len=%zu aosp=%zu samsung=%zu error=%d",
                slotId, response, responseLen, aospSize, samsungSize, e);

        if (response == NULL ||
                (responseLen != aospSize && responseLen != samsungSize)) {
            RLOGE("getIccCardStatusResponse: unsupported response size %zu", responseLen);
            if (e == RIL_E_SUCCESS) {
                responseInfo.error = RadioError::INVALID_RESPONSE;
            }
        } else {
            const RIL_CardStatus_Header_Compat *header =
                    static_cast<const RIL_CardStatus_Header_Compat *>(response);

            RLOGV("ICC HDR slot=%d state=%d pin=%d gsm=%d cdma=%d ims=%d apps=%d",
                    slotId,
                    header->card_state,
                    header->universal_pin_state,
                    header->gsm_umts_subscription_app_index,
                    header->cdma_subscription_app_index,
                    header->ims_subscription_app_index,
                    header->num_applications);

            if (header->num_applications < 0 ||
                    header->num_applications > RIL_CARD_MAX_APPS) {
                RLOGE("getIccCardStatusResponse: invalid application count %d",
                        header->num_applications);
                if (e == RIL_E_SUCCESS) {
                    responseInfo.error = RadioError::INVALID_RESPONSE;
                }
            } else {
                cardStatus.cardState = (CardState) header->card_state;
                cardStatus.universalPinState = (PinState) header->universal_pin_state;
                ...
                cardStatus.imsSubscriptionAppIndex =
                        header->ims_subscription_app_index;
                cardStatus.applications.resize(header->num_applications);

                AppStatus *appStatus = cardStatus.applications.data();

                if (responseLen == aospSize) {
                    const RIL_CardStatus_v6_AOSP_Compat *p_cur =
                            static_cast<const RIL_CardStatus_v6_AOSP_Compat *>(response);

                    for (int i = 0; i < header->num_applications; i++) {
                        const RIL_AppStatus_AOSP_Compat& rilAppStatus =
                                p_cur->applications[i];

                        appStatus[i].appType = (AppType) rilAppStatus.app_type;
                        appStatus[i].appState = (AppState) rilAppStatus.app_state;
                        appStatus[i].persoSubstate =
                                (PersoSubstate) rilAppStatus.perso_substate;
                        appStatus[i].aidPtr =
                                convertCharPtrToHidlString(rilAppStatus.aid_ptr);
                        appStatus[i].appLabelPtr =
                                convertCharPtrToHidlString(rilAppStatus.app_label_ptr);
                        appStatus[i].pin1Replaced = rilAppStatus.pin1_replaced;
                        appStatus[i].pin1 = (PinState) rilAppStatus.pin1;
                        appStatus[i].pin2 = (PinState) rilAppStatus.pin2;
                    }
                } else {
                    const RIL_CardStatus_v6 *p_cur =
                            static_cast<const RIL_CardStatus_v6 *>(response);

                    for (int i = 0; i < header->num_applications; i++) {
                        const RIL_AppStatus& rilAppStatus = p_cur->applications[i];
...
int radio::getSignalStrengthResponse(int slotId,
    ...
    if (radioService[slotId]->mRadioResponse != NULL) {
        ...
        RLOGV("SIGNAL ABI slot=%d response=%p len=%zu v5=%zu v6=%zu v8=%zu v10=%zu error=%d",
                slotId, response, responseLen,
        ...
        if (response == NULL ||
            ...
            RLOGE("getSignalStrengthResponse: unsupported response size %zu", responseLen);
            if (e == RIL_E_SUCCESS) {
                responseInfo.error = RadioError::INVALID_RESPONSE;
            }
        } else {
            /*
             * Samsung blobs may append private fields after the standard v10
             * structure. The standard fields remain a compatible prefix.
             */
            const size_t compatibleLen =
                    responseLen > sizeof(RIL_SignalStrength_v10)
                    ? sizeof(RIL_SignalStrength_v10)
                    : responseLen;
            convertRilSignalStrengthToHal(response, compatibleLen, signalStrength);
        }
...
/*
 * Qualcomm/Samsung Android 9 RIL uses fixed char[4] MCC/MNC fields.
 * Copy at most three digits and never trust missing NUL termination.
 */
static std::string convertRilPlmnValue(const char *rawValue) {
    if (rawValue == NULL) {
        return std::string();
    }

    const size_t length = strnlen(rawValue, MAX_RIL_MCC_MNC_SIZE);
    ...
    return std::string(rawValue, length);
}
    ...
    switch(rilCellIdentity.cellInfoType) {
        ...
        case RIL_CELL_INFO_TYPE_GSM: {
            ...
            cellIdentity.cellIdentityGsm[0].mcc =
                    convertRilPlmnValue(rilCellIdentity.cellIdentityGsm.mcc);
            cellIdentity.cellIdentityGsm[0].mnc =
                    convertRilPlmnValue(rilCellIdentity.cellIdentityGsm.mnc);
        ...
        case RIL_CELL_INFO_TYPE_WCDMA: {
            ...
            cellIdentity.cellIdentityWcdma[0].mcc =
                    convertRilPlmnValue(rilCellIdentity.cellIdentityWcdma.mcc);
            cellIdentity.cellIdentityWcdma[0].mnc =
                    convertRilPlmnValue(rilCellIdentity.cellIdentityWcdma.mnc);
        ...
        case RIL_CELL_INFO_TYPE_LTE: {
            ...
            cellIdentity.cellIdentityLte[0].mcc =
                    convertRilPlmnValue(rilCellIdentity.cellIdentityLte.mcc);
            cellIdentity.cellIdentityLte[0].mnc =
                    convertRilPlmnValue(rilCellIdentity.cellIdentityLte.mnc);
        ...
        case RIL_CELL_INFO_TYPE_TD_SCDMA: {
            ...
            cellIdentity.cellIdentityTdscdma[0].mcc =
                    convertRilPlmnValue(rilCellIdentity.cellIdentityTdscdma.mcc);
            cellIdentity.cellIdentityTdscdma[0].mnc =
                    convertRilPlmnValue(rilCellIdentity.cellIdentityTdscdma.mnc);
...
static bool looksLikeLegacyRegistrationStringArray(
        void *response, size_t responseLen, size_t minimumStrings) {
    if (response == NULL ||
            responseLen < minimumStrings * sizeof(char *) ||
            responseLen % sizeof(char *) != 0) {
        return false;
    }

    char **strings = static_cast<char **>(response);
    const size_t count = responseLen / sizeof(char *);
    const size_t inspectCount = count < 6 ? count : 6;
    size_t plausiblePointers = 0;

    for (size_t i = 0; i < inspectCount; i++) {
        const uintptr_t value = reinterpret_cast<uintptr_t>(strings[i]);
        if (value == 0) {
            continue;
        }

        /*
         * Integer-based registration structs start with tiny enum values.
         * A legacy char ** response contains userspace pointers instead.
         */
        if (value < 4096) {
            return false;
        }
...
int radio::getVoiceRegistrationStateResponse(int slotId,
    ...
    if (radioService[slotId]->mRadioResponse != NULL) {
        ...
        const int numStrings = responseLen / sizeof(char *);
        const bool legacyStrings = looksLikeLegacyRegistrationStringArray(
                response, responseLen, 15);

        RLOGV("VOICE REG ABI slot=%d response=%p len=%zu struct=%zu strings=%d "
                "legacy=%d rilVersion=%d error=%d",
                slotId, response, responseLen,
                sizeof(RIL_VoiceRegistrationStateResponse),
                numStrings, legacyStrings, s_vendorFunctions->version, e);

        if (response == NULL) {
            RLOGE("getVoiceRegistrationStateResponse: NULL response");
            if (e == RIL_E_SUCCESS) {
                responseInfo.error = RadioError::INVALID_RESPONSE;
            }
        } else if (legacyStrings) {
            ...
            fillCellIdentityFromVoiceRegStateResponseString(
                    voiceRegResponse.cellIdentity, numStrings, resp);
        } else if (responseLen >= sizeof(RIL_VoiceRegistrationStateResponse)) {
            /*
             * Accept Samsung structures with private fields appended after
             * the standard registration-state prefix.
             */
            RIL_VoiceRegistrationStateResponse *voiceRegState =
                    static_cast<RIL_VoiceRegistrationStateResponse *>(response);
            ...
            fillCellIdentityResponse(
                    voiceRegResponse.cellIdentity, voiceRegState->cellIdentity);
        } else {
            RLOGE("getVoiceRegistrationStateResponse: unsupported response size %zu",
                    responseLen);
            if (e == RIL_E_SUCCESS) {
                responseInfo.error = RadioError::INVALID_RESPONSE;
            }
...
int radio::getDataRegistrationStateResponse(int slotId,
    ...
    if (radioService[slotId]->mRadioResponse != NULL) {
        ...
        const int numStrings = responseLen / sizeof(char *);
        const bool legacyStrings = looksLikeLegacyRegistrationStringArray(
                response, responseLen, 6);

        RLOGV("DATA REG ABI slot=%d response=%p len=%zu struct=%zu strings=%d "
                "legacy=%d rilVersion=%d error=%d",
                slotId, response, responseLen,
        ...
        if (response == NULL) {
            RLOGE("getDataRegistrationStateResponse: NULL response");
            if (e == RIL_E_SUCCESS) {
                responseInfo.error = RadioError::INVALID_RESPONSE;
            }
        } else if (legacyStrings) {
            ...
            fillCellIdentityFromDataRegStateResponseString(
                    dataRegResponse.cellIdentity, numStrings, resp);
        } else if (responseLen >= sizeof(RIL_DataRegistrationStateResponse)) {
            /*
             * Accept Samsung structures with private fields appended after
             * the standard registration-state prefix.
             */
            RIL_DataRegistrationStateResponse *dataRegState =
                    static_cast<RIL_DataRegistrationStateResponse *>(response);
            ...
            fillCellIdentityResponse(
                    dataRegResponse.cellIdentity, dataRegState->cellIdentity);
        } else {
            RLOGE("getDataRegistrationStateResponse: unsupported response size %zu",
                    responseLen);
            if (e == RIL_E_SUCCESS) {
                responseInfo.error = RadioError::INVALID_RESPONSE;
            }
...
int radio::getCellInfoListResponse(int slotId,
    ...
    if (radioService[slotId]->mRadioResponse != NULL) {
        ...
        RLOGV("CELL INFO ABI slot=%d response=%p len=%zu record=%zu error=%d",
                slotId, response, responseLen, sizeof(RIL_CellInfo_v12), e);

        if ((response == NULL && responseLen != 0) ||
                responseLen % sizeof(RIL_CellInfo_v12) != 0) {
            RLOGE("getCellInfoListResponse: invalid J415 response len=%zu",
                    responseLen);
            if (e == RIL_E_SUCCESS) {
                responseInfo.error = RadioError::INVALID_RESPONSE;
            }
...
int radio::currentSignalStrengthInd(int slotId,
                                    ...
                                    void *response, size_t responseLen) {
    if (radioService[slotId] != NULL && radioService[slotId]->mRadioIndication != NULL) {
        RLOGV("SIGNAL IND ABI slot=%d response=%p len=%zu v8=%zu v10=%zu error=%d",
                slotId, response, responseLen,
                sizeof(RIL_SignalStrength_v8),
                sizeof(RIL_SignalStrength_v10),
                e);
        ...
        if (response == NULL ||
                (responseLen != sizeof(RIL_SignalStrength_v8) &&
                 responseLen < sizeof(RIL_SignalStrength_v10))) {
            RLOGE("currentSignalStrengthInd: unsupported response size %zu", responseLen);
            return 0;
        }
        ...
        switch(rillCellInfo->cellInfoType) {
            case RIL_CELL_INFO_TYPE_GSM: {
                ...
                cellInfoGsm->cellIdentityGsm.mcc =
                        convertRilPlmnValue(rillCellInfo->CellInfo.gsm.cellIdentityGsm.mcc);
                cellInfoGsm->cellIdentityGsm.mnc =
                        convertRilPlmnValue(rillCellInfo->CellInfo.gsm.cellIdentityGsm.mnc);
            ...
            case RIL_CELL_INFO_TYPE_WCDMA: {
                ...
                cellInfoWcdma->cellIdentityWcdma.mcc =
                        convertRilPlmnValue(rillCellInfo->CellInfo.wcdma.cellIdentityWcdma.mcc);
                cellInfoWcdma->cellIdentityWcdma.mnc =
                        convertRilPlmnValue(rillCellInfo->CellInfo.wcdma.cellIdentityWcdma.mnc);
            ...
            case RIL_CELL_INFO_TYPE_LTE: {
                ...
                cellInfoLte->cellIdentityLte.mcc =
                        convertRilPlmnValue(rillCellInfo->CellInfo.lte.cellIdentityLte.mcc);
                cellInfoLte->cellIdentityLte.mnc =
                        convertRilPlmnValue(rillCellInfo->CellInfo.lte.cellIdentityLte.mnc);
            ...
            case RIL_CELL_INFO_TYPE_TD_SCDMA: {
                ...
                cellInfoTdscdma->cellIdentityTdscdma.mcc =
                        convertRilPlmnValue(rillCellInfo->CellInfo.tdscdma.cellIdentityTdscdma.mcc);
                cellInfoTdscdma->cellIdentityTdscdma.mnc =
                        convertRilPlmnValue(rillCellInfo->CellInfo.tdscdma.cellIdentityTdscdma.mnc);
...
int radio::cellInfoListInd(int slotId,
                           ...
                           size_t responseLen) {
    if (radioService[slotId] != NULL && radioService[slotId]->mRadioIndication != NULL) {
        RLOGE("CELL INFO IND ABI slot=%d response=%p len=%zu record=%zu error=%d",
                slotId, response, responseLen, sizeof(RIL_CellInfo_v12), e);

        if ((response == NULL && responseLen != 0) ||
                responseLen % sizeof(RIL_CellInfo_v12) != 0) {
            RLOGE("cellInfoListInd: invalid J415 response len=%zu", responseLen);
            return 0;
        }
